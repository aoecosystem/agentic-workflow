...
**One product, one team, one repo. Backend monolith + frontend SPA + shared dev infra inside `apps/`.**
...
(compose, dev docker images, scripts) sits at `apps/infra/`, next to
`apps/api/` and `apps/web/`. The backend itself is monolithic — one API
...
│   ├── infra/                            ← dev infra (NOT at repo root; lives next to api + web)
│   │   ├── compose/
│   │   │   ├── docker-compose.dev.yml    ← FE + BE + dev DB + Redis
│   │   │   ├── docker-compose.staging.yml
│   │   │   └── docker-compose.prod.yml   ← uses managed DB (Neon/RDS/Supabase) — only app images
│   │   ├── docker/                       ← DEV ONLY images
│   │   │   ├── postgres/                 ← Dockerfile + init/ + postgres.conf
│   │   │   ├── redis/                    ← Dockerfile + redis.conf
│   │   │   └── nginx/                    ← (optional) reverse proxy in front of api
│   │   ├── environments/                 ← per-service env files
│   │   │   ├── postgres.env
│   │   │   ├── redis.env
│   │   │   └── api.env
│   │   └── scripts/
│   │       ├── dev.sh                    ← starts compose + tails logs
│   │       ├── deploy.sh
│   │       ├── health.sh
│   │       └── rollback.sh
│   │
...
`apps/web/Dockerfile`). `apps/infra/` orchestrates them but does not
own them.
...
5. **`apps/infra/` lives inside `apps/`**, next to `api/` and `web/`. NOT at repo root.
...
|---|---|---|
| **Reverse proxy** in front of API | `apps/infra/docker/nginx/` | TLS, rate limit, route `/api/*` → api:3000, `/*` → web:80 |
| **SPA static server** for FE (Vite only) | `apps/web/nginx.conf` | Serves `apps/web/dist/` inside the web Dockerfile |

Drop `apps/infra/docker/nginx/` if deploying behind a managed LB
...
- **Dev DB:** dockerized via `apps/infra/docker/postgres/`.
