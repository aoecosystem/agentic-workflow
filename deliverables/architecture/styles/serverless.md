...
- **Local dev:** SST Live | Vercel CLI | Wrangler (Cloudflare) | docker-compose for local DB
...
- **Schema migrations:**
  - SQL DBs: tool depends on ORM (Drizzle Kit / Prisma Migrate / Atlas) — run via `infra/scripts/migrate.sh` in CI before deploy.
